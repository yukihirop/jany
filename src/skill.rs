//! Ships the `/jany-register` and `/jany-update` skills, the `jany --register <name>` scaffold,
//! and `jany --update`, which brings the built-in definitions up to date.
//! The skills (`skills/<locale>/jany-{register,update}/`, en / ja) are embedded in the binary and written on every
//! `jany --init` to `~/.agents/skills/` (a place both Claude Code and Codex can read).
...
/// The skills for one language (`skills/<locale>/jany-register/`, `skills/<locale>/jany-update/`).
