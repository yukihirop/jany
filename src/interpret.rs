...
pub fn run(schema: &Schema, cfg: &Config, words: &[String], passthrough: &[String], oracle: Option<&dyn Oracle>, defaults_override: Option<&toml::Table>) -> Result<Run, JxError> {
    ...
    if tokens.iter().any(|t| !t.resolved()) {
        let Some(oracle) = oracle else {
            ...
            return Err(JxError::Unresolved(format!("{} (jev disabled)", bad.join(", "))));
        };
...
pub fn oracle_from_config(cfg: &Config) -> Result<jev::client::OpenRouter, JxError> {
    let api_key = std::env::var("OPENROUTER_API_KEY")
        .ok()
        .filter(|k| !k.is_empty())
        .or_else(|| cfg.jev.api_key.clone())
        .ok_or_else(|| JxError::Jev("no API key. run `jx setup` or set OPENROUTER_API_KEY (needed to interpret ambiguous words)".into()))?;
