...
- `risk`: `"none"` | `"unsafe"`(取り消しにくい: PUT/DELETE)| `"dangerous"`(破壊的: rm、find -delete)。config.toml に `[cmd.<name>] autorun = true` があると、規則だけで決まった `"none"` の行は入力行で見ずに実行される。状態を変えるもの(インストール、ファイルの書き込み、データの送信)は `"unsafe"` にする
