...
- **`jany --on` / `--off`**(2026-09-24、ユーザー指示「mahojin の --on / --off を参考に」、選択は推奨案「jany を付けずに打てる」): mahojin(`~/RustProjects/mahojin/src/shell.rs`)と同じ形。zsh ラッパーの `jany()` が引数 1 個の `--on` / `--off` だけを横取りして `_JANY_ON` を立てる/消す。`accept-line` を `_jany_accept_line` で包み(元が user widget なら `_jany_accept_line_orig` に退避して呼ぶ)、on の間は Enter で `jany --claim -- <語>`(`src/claim.rs`、終了コード 0/1、config も jev も読まない)が 0 を返した行の頭に `jany ` を足す。履歴には `jany find …` が残る。引き受けるのは「定義のあるコマンドで始まり、その後に語がある」行だけで、`-` で始まる語・`|` `&&` `;` `>` `2>` `<(…)`・コマンド単体(`find`)・定義の無いサブコマンド(`docker ps`)・先頭の語がクォートされた行(`\find` `'find'` `command find`)は素通し。on の間は薄い候補も `find src ` に出る(`-` の語がある行は出さない)。バイナリまで届いた `--on` / `--off`(ラッパー無し、bash / fish)は「zsh のラッパーで使う」と言って 64 で終わる。`script` で擬似端末を作り `zsh -f -i` に打鍵を流して、書き換え・素通し・`--off`・履歴・薄い候補を確認した(zpty は今回も読み取りで止まった)。**実端末では試していない**。**`pnpm install` `kubectl get pods` のように `-` の無い普段の打ち方も jany に回る**(autorun が効かなければ入力行に置かれてもう 1 回 Enter)。除外リストは `config.toml` の `[on] skip = ["kubectl", "docker compose"]`(同日、ユーザー指示「やってくれ」。行の先頭と語単位で比べる。既定は空。薄い候補も `jany --suggest --on` で同じリストを見て出さない)。同じ日に、新しい clippy で `src/questions.rs:136` が `nonminimal_bool` で落ちるようになっていたので直した
- **`/jany-setup` スキルと `jany --skills`**(2026-09-24、ユーザー指示「初回のセットアップを agent の skill で」、届け方は推奨案「--skills を新設」): `skills/{ja,en}/jany-setup/SKILL.md` の 1 ファイル。AskUserQuestion でシェル(zsh / bash / fish、`$SHELL` を推奨)を聞き、rc に `jany --init` の行を書き(既にあれば足さない)、`jany --init <shell> >/dev/null` で定義とスキルを置き、zsh なら `jany --on 2>/dev/null` を書くかも AskUserQuestion で聞く(推奨は「書く」)。API キーは有無だけ見て、無ければ `! jany --setup`。ja 版は rc の行に `--locale ja` を付ける。初回はまだスキルが無いので、`jany --skills [--locale]` がスキルだけを置く(`skill::install` を呼ぶだけ。rc も定義も触らない)。`--init` / `--update` でも jany-setup が入る。scratch の HOME で `--skills` の配置・symlink・2 回目に何も書かないことを確認。**`/jany-setup` をスキルとして呼んで試してはいない**。同日、README のセットアップは `cargo install jany` → `jany --skills` → `/jany-setup` だけにし、手での設定・薄い候補・autorun・`jany --on`(on.svg)・API キーは `docs/setup.md` / `docs/setup.ja.md` に移した(ユーザー指示「README をシンプルに」)。続けてフラグ表と config.toml を `docs/usage(.ja).md`、「コマンドを足す」「定義を新しくする」を `docs/commands(.ja).md` に移した(README は 188 → 108 行。「仕組み」「弱いところ」は README に残した)
- **README をさらに短く**(2026-09-24、ユーザー指示「仕組みも別 doc に」「始め方 → jany-setup、やめ方 → jany-teardown が一発で目に入る」): 「仕組み」「弱いところ」を `docs/how-it-works(.ja).md` に移し、セットアップ・使い方・コマンドを足すを「はじめ方」の表 1 つ(始める / 使う / コマンドを足す / jany を上げたら / やめる)と「もっと詳しく」のリンク 4 本にした。README は 79 行。続けて、demo.svg の下にあった例のコードブロックと「最後の行は出力ではない」の段落を `docs/examples(.ja).md` に、jind/jurl を汎化した旨の段落を `docs/how-it-works(.ja).md` の冒頭に移した(ユーザー指示「gif があるから例は別ドキュメントに」)。README は 50 行
- **`/jany-teardown` スキル**(2026-09-24、ユーザー指示「jany を辞める時のスキル」): `skills/{ja,en}/jany-teardown/SKILL.md`。rc の jany の行(`--init` / `--on` / jany を指す alias・compdef。jany を含むだけの行は拾わない)、`~/.config/jany/`、スキルと symlink、バイナリを、AskUserQuestion(multiSelect)で選ばせ、一覧を見せてもう一度確認してから片付ける。rc は `.jany-teardown.bak` を取ってから行を消し構文チェック、`~/.config/jany` は既定で `~/jany-backup-<日付>/` に退避(自作の定義と API キーを守るため)。順番は rc → 設定 → symlink → スキル(自分は最後)→ `cargo uninstall`(rc を先に外さないと `--init` がスキルを置き直す)。`--init` / `--skills` / `--update` で入る。**実際に呼んで試してはいない**
