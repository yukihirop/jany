...
- **出力** — コマンド行は stdout、それ以外は stderr。確信度が下限を下回ったら何も出さず非ゼロで終わる。`dangerous` な結果(find の `-delete`)は先に read-only で実行してプレビューする。`unsafe` なもの(curl の `DELETE`、docker の `--privileged`)には 1 行の注意が付く。
...
reject_below = 0.5           # これ未満なら何も出さず非ゼロで終わる
